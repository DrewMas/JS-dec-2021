...
for (const item of usersList) {
    ...
    let divCreator = (item, div) => {
        for (const el in item) {
            let divka = document.createElement(`div`);
            divka.innerText = `${el} - ${item[el]}`;
            divka.style.margin = `30px`;
            divka.style.border = `3px solid gold`;
            divka.style.paddingLeft = `10px`;
            div.append(divka);
            if (typeof item[el] === `object`){
                divka.innerText = `${el}:`;
                divCreator(item[el], divka);
            }
        }
    }
    divCreator(item, div);
